...
Notes:      Version 4: Block AI + Block human, d = orange, s = blue
"""
...
ratingScale = visual.RatingScale(win, 
                                 low = 1, 
                                 high = 4,
                                 marker='circle', stretch = 2,markerStart = 2.5,
                                 tickMarks = [1, 2, 3, 4], 
                                 tickHeight = 1.5,  
                                 singleClick = True,
...
# Create a function to display instructions and breaks
...
def grid_f (re_elements_b): # The argument that can be defined is the number of blue pixels
    ...
    # Use all of it to create the visual element                    
...
for idx, row in cond_df_ai.iterrows():
    ...
    # Save trail info
    AIexp.addData('Version', '4(BlockAI+Blockhuman,d=orange,s =blue)')
    ...
    if curr_blue_nr == 8192:
        accuracy = 'NA'
    elif (response == 'd') and (curr_blue_nr == 7782):
        accuracy = 1
    elif (response == 's') and (curr_blue_nr == 8602):
        accuracy = 1
    else:
        accuracy = 0 
